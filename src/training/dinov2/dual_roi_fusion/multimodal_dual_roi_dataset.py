...
class MultimodalDualROIDataset(Dataset):
    """
    Dataset for 4-1 Original US + ROI US dual-branch experiment.

    Inputs:
        1. Mammography whole image
        2. Original ultrasound whole image
        3. Ultrasound ROI crop image
    ...
    def __init__(
        ...
        split="train",
        image_size=224,
        roi_col="ultrasound_roi_path",
    ):
        logging.info(f"Loading dual-ROI multimodal metadata from: {csv_file} for split: '{split}'")
        ...
        logging.info(f"Loaded {len(self.split_df)} dual-ROI pairs for split '{split}'")
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(us_path):
            raise FileNotFoundError(f"Original ultrasound image not found: {us_path}")
